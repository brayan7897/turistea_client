...
//const {modal , mostrarModal} = generalsContext;
  ...
  const vModal = modal ? "absolute top-0 left-0 w-full h-hidden z-50 bg-colorwo p-4 overflow-x-hidden" : "hidden";
  ...
  return(
   ...
   <div>
      ...
      <div  className=" w-80 h-80  bg-colorwo rounded-2xl grid grid-rows-2">
        <span className="w-full h-1/4"><img src={card.imageUrl.stringValue} alt='imagen no disponible' className="w-full rounded-t-2xl object-cover"></img> </span>
          <div className="flex flex-col justify-between text-colorb">
            <div className="px-3 pt-20 gap-2">
              <p className="text-left font-bold text-sm">{card.title.stringValue}</p>
              <p className="text-left font-normal text-xs">{card.buttonUrl.stringValue}</p>
            </div>
      ...
      <span  className="final w-full h-1/4"><img src={card.imageUrl.stringValue} alt='imagen no disponible' className="w-full rounded-2xl object-cover"></img> </span>
         <p className="text-colorwc text-center py-4 font-bold">{card.title.stringValue}</p>
         ...
         <p className="text-justify text-colorwc">{card.buttonText.stringValue}</p>
